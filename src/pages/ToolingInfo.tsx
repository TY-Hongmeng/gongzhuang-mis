...
const ToolingInfoPage: React.FC = () => {
  ...
  const syncLocalToolingTotals = useCallback((toolingId: string, _retryCount: number = 0) => {
    ...
    if (savedTotalsSuccessRef.current.has(normalizedId)) {
      console.log(`[Totals] ⏭️ 跳过已保存的工具 ${normalizedId}，保持当前显示值不变`)
      return
    }
    ...
    const getRouteProgressStatus = (rec: PartItem, routeText: string) => {
      ...
      if (steps.length === 0) {
        ...
        if (latestEntry) {
          ...
          if (totalHours > 0) {
            return { text: actorText ? `${actorText}加工中 ${totalHours.toFixed(1)}h` : `加工中 ${totalHours.toFixed(1)}h`, color: '#1890ff', hasRoute: false }
          }
        ...
        if (totalHours > 0) {
          return { text: `加工中 ${totalHours.toFixed(1)}h`, color: '#1890ff', hasRoute: false }
        }
        return { text: '加工中', color: '#1890ff', hasRoute: false }
      }
      function statesFromRoute(stepsRaw: string[]) {
        return stepsRaw.map((step, index) => {
          ...
          const inProgressByWork = stepQty > 0 || stepHours > 0
          ...
          return { step, key, token, stepQty, stepHours, doneByQty, inProgressByWork, inProgress, manualChecked, latestMeta }
        })
      }
    ...
    return [
      ...
      },
      {
        ...
        render: (_text: any, rec: PartItem) => {
          ...
          return <span style={{ color: '#000000' }}>{Number.isFinite(calculatedAmount) ? calculatedAmount.toFixed(2) : '0.00'}</span>
        }
      },
      {
        title: '状态',
  ...
  }, [renderStatusText, saveStatusInput, workHoursData, workHoursProcessCompletedQtyData, workHoursProcessHoursData, workHoursProcessAmountData, workHoursProcessLatestMetaData, manualStepUpdateMap, user])
