...
const ToolingInfoPage: React.FC = () => {
  ...
  const syncLocalToolingTotals = useCallback((toolingId: string) => {
    ...
    if (!normalizedId.startsWith('blank-')) {
      fetchWithFallback(`/api/tooling/${encodeURIComponent(normalizedId)}/refresh-totals`, {
        ...
        if (!response.ok) {
          console.warn(`保存工具 ${normalizedId} 总额失败:`, response.status)
        }
      }).catch(err => {
        console.warn(`保存工具 ${normalizedId} 总额异常:`, err)
      })
    }
  ...
  const { filteredVisibleData, counts } = useMemo(() => {
    ...
    // 排序逻辑：首先按级别星级降序，然后按盘存编号排序（提取数字部分），字母不同的按接收日期、需求日期排序
    result = result.sort((a: any, b: any) => {
      // 1. 首先按级别星级降序（星级越高排越前）
      const priorityA = Number(a.priority_level || 0)
      const priorityB = Number(b.priority_level || 0)
