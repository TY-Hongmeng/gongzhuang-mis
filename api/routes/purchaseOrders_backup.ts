...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const processedData = (data || []).map((item: any) => {
      // 从tooling_info中提取数据并合并到主记录中
      const toolingInfo = item.tooling_info?.[0] || {};
      return {
        ...item,
        production_unit: item.production_unit || toolingInfo.production_unit || '未知单位',
        applicant: item.applicant || toolingInfo.recorder || '未知录入人'
        // 注意：不再使用tooling_info.demand_date，只使用purchase_orders自身的demand_date
      };
    });
      ...
      for (const order of orders) {
            ...
            if (hasChanges) {
              ...
              const updateResult = await queryDatabase(`
                ...
                SET inventory_number = $1, project_name = $2, part_name = $3, part_quantity = $4, 
                    unit = $5, model = $6, supplier = $7, required_date = $8, remark = $9, 
                    updated_date = $10, production_unit = $11, demand_date = $12, applicant = $13
                WHERE tooling_id = $14 AND part_name = $15 AND part_id IS NULL
                RETURNING *
              `, [
                order.inventory_number,
                order.project_name,
        ...
        if (order.tooling_id) {
          try {
            ...
            if (toolingResult.rows.length > 0) {
              productionUnit = toolingResult.rows[0].production_unit || '未知单位';
              recorder = toolingResult.rows[0].recorder || '未知录入人';
            }
          } catch (error) {
            console.warn(`[PurchaseOrders] Failed to fetch tooling_info for tooling_id ${order.tooling_id}:`, error);
          }
...
// GET /api/purchase-orders/manual
// 获取手动输入的采购单 - 返回所有采购单，由前端进行筛选
router.get('/manual', async (req, res) => {
  try {
    console.log('=== 获取手动采购单开始 ===');
    ...
    const mappedData = (data || []).map(item => {
      const mappedItem = {
        ...
        demand_date: item.required_date || item.demand_date || '',
        // 确保其他字段也正确映射
        applicant: item.applicant || item.recorder || '',
        part_quantity: item.part_quantity || 0
      };
      ...
      console.log('字段映射详情:', {
        ...
        demand_date: mappedItem.demand_date,
        applicant: mappedItem.applicant,
        part_quantity: mappedItem.part_quantity
      });
      
      return mappedItem;
    });

    console.log('最终映射数据长度:', mappedData.length);
    ...
    console.error('获取手动采购单路由错误:', err);
    res.status(500).json({ success: false, error: '服务器错误' });
  }
});

// PUT /api/purchase-orders/manual/:id
// 更新手动输入的采购单
router.put('/manual/:id', async (req, res) => {
  try {
    ...
    // 验证必填字段
    if (updates.part_name !== undefined && !updates.part_name?.trim()) {
      return res.status(400).json({ success: false, error: '名称不能为空' });
    }

    if (updates.part_quantity !== undefined && (!updates.part_quantity || parseInt(updates.part_quantity) <= 0)) {
      return res.status(400).json({ success: false, error: '数量必须是正整数' });
    }

    if (updates.unit !== undefined && !updates.unit?.trim()) {
      return res.status(400).json({ success: false, error: '单位不能为空' });
    }
    ...
    if (!data) {
      return res.status(404).json({ success: false, error: '未找到指定的采购单' });
    }
    ...
    const mappedData = {
      ...data,
      // 将数据库的 required_date 映射为前端的 demand_date
      demand_date: data.required_date || data.demand_date || '',
      // 确保其他字段也正确映射
      applicant: data.applicant || data.recorder || '',
      part_quantity: data.part_quantity || 0
    };
    ...
    console.error('更新手动采购单路由错误:', err);
    res.status(500).json({ success: false, error: '服务器错误' });
  }
});

// DELETE /api/purchase-orders/manual/:id
// 删除手动输入的采购单
router.delete('/manual/:id', async (req, res) => {
  try {
    ...
    console.error('删除手动采购单路由错误:', err);
    res.status(500).json({ success: false, error: '服务器错误' });
  }
});

// POST /api/purchase-orders/manual/batch-delete
// 批量删除手动输入的采购单
router.post('/manual/batch-delete', async (req, res) => {
  try {
    const { ids } = req.body;
    ...
    if (error) {
      console.error('批量删除手动采购单失败:', error);
      return res.status(500).json({ success: false, error: error.message });
    }
    ...
    console.error('批量删除手动采购单路由错误:', err);
    res.status(500).json({ success: false, error: '服务器错误' });
  }
});
