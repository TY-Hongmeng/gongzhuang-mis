    ...
    const sql = `
      WITH inbound AS (
        SELECT
          ...
          location,
          unit,
          SUM(CASE WHEN status <> '已删除' THEN quantity ELSE 0 END) AS inbound_total,
          MAX(CASE WHEN status <> '已删除' THEN unit_price ELSE 0 END) AS latest_inbound_price
      ...
      outbound AS (
        SELECT
          ...
          location,
          unit,
          SUM(CASE WHEN status <> '已删除' THEN quantity ELSE 0 END) AS outbound_total,
          SUM(CASE WHEN status <> '已删除' THEN quantity ELSE 0 END) AS total_used_qty,
          MIN(CASE WHEN status <> '已删除' THEN out_date END) AS min_out_date,
          MAX(CASE WHEN status <> '已删除' THEN out_date END) AS max_out_date
    ...
    const inserted = await transaction(async (client) => {
      ...
      for (const raw of items) {
        ...
        const stockRow = await client.query(`
          SELECT
            COALESCE((SELECT SUM(quantity) FROM standard_part_inbound WHERE status <> '已删除' AND name=$1 AND spec_model=$2 AND tech_group=$3 AND location=$4 AND unit=$5), 0)
            -
            COALESCE((SELECT SUM(quantity) FROM standard_part_outbound WHERE status <> '已删除' AND name=$1 AND spec_model=$2 AND tech_group=$3 AND location=$4 AND unit=$5), 0)
            AS balance
